...
mounted workspace volumes remain writable. ``TMPDIR`` and ``XDG_CACHE_HOME``
are redirected to per-job directories inside the workspace (unless the user
already set them), so software that hardcodes paths like ``/tmp`` will fail.
Host-path mounts default to read-only unless a mount sets ``readOnly: false``.
"""
